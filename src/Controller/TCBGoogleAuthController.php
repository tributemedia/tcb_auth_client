...
use Drupal\tcb_auth_client\Controller\TCBUserAuthenticator;

/**
 * Returns responses for Social Auth Google module routes.
 */
class TCBGoogleAuthController extends OAuth2ControllerBase {

  /**
   * TCBGoogleAuthController constructor.
   *
  ...
  public function __construct(MessengerInterface $messenger,
    ...
    parent::__construct('Social Auth Google', 'social_auth_google', $messenger,
      $network_manager, $user_authenticator, $google_manager, $request,
      $data_handler);
    $this->userAuthenticator = 
      new TCBUserAuthenticator($this->userAuthenticator);
  }
  ...
  public function callback() {
    ...
    if ($profile !== NULL) {
      ...
      $data = $this->userAuthenticator->
        checkProviderIsAssociated($profile->getId()) ? 
        NULL : $this->providerManager->getExtraDetails();

      return $this->userAuthenticator->authenticateUser($profile->getName(), 
        $profile->getEmail(), $profile->getId(), 
        $this->providerManager->getAccessToken(), $profile->getAvatar(), 
        $data);
    }
